...
const writtenIllustrations = [
  { concept: "conceito principal", text: "Imagine uma situação simples do dia a dia que ajude a compreender a ideia central deste tema." },
  { concept: "aplicação prática", text: "Uma maneira de aplicar este princípio é escolher uma atitude concreta e praticá-la durante a semana." },
  { concept: "família", text: "Uma família que conversa com respeito e coopera nas decisões demonstra como este tema pode fortalecer o lar." },
  { concept: "jovens", text: "Um jovem pode usar este princípio ao tomar decisões na escola, nas amizades e no uso do seu tempo." },
  { concept: "oração", text: "Ao enfrentar uma dificuldade, reservar um momento para orar pode ajudar a buscar orientação e manter a calma." },
  { concept: "estudo bíblico", text: "Ler um relato bíblico e refletir sobre as escolhas dos personagens ajuda a tornar o aprendizado mais pessoal." },
  { concept: "reunião cristã", text: "A participação em uma reunião cristã oferece encorajamento e oportunidades para colocar em prática o que foi aprendido." },
  { concept: "natureza", text: "Observar a ordem e a beleza da natureza pode despertar gratidão e reforçar a confiança no Criador." },
  { concept: "ajuda ao próximo", text: "Oferecer tempo, atenção ou ajuda prática a alguém em necessidade é uma forma de demonstrar amor genuíno." },
  { concept: "decisão sábia", text: "Antes de decidir, considerar os princípios envolvidos e as possíveis consequências favorece uma escolha equilibrada." },
  { concept: "confiança", text: "Mesmo sem conhecer todos os detalhes do futuro, é possível avançar com confiança quando se segue uma orientação segura." },
  { concept: "esperança", text: "A esperança funciona como uma âncora: ajuda a permanecer firme quando as circunstâncias atuais são difíceis." },
  { concept: "amor", text: "O amor verdadeiro aparece em ações pacientes, bondosas e atenciosas, especialmente quando seria mais fácil desistir." },
  { concept: "fé", text: "A fé pode ser comparada a uma ponte: permite agir com confiança mesmo antes de enxergar todo o caminho." },
  { concept: "vida cotidiana", text: "Pequenas escolhas feitas em casa, no trabalho e nas conversas diárias podem revelar como este tema orienta a vida." },
];
      ...
      <div className="mt-3 grid gap-2 sm:grid-cols-2 md:grid-cols-3">
        {writtenIllustrations.map((illustration, index) => (
          <article key={`${themeNum}-${illustration.concept}`} className="rounded-lg border border-[#e2dccf] bg-white p-3">
            ...
            <p className="mt-2 text-xs leading-relaxed text-[#405653]">{illustration.text} Tema relacionado: {title}.</p>
          </article>
        ))}
      </div>
    </details>
...
function Home() {
  ...
  const incompleteAgendaCount = useMemo(
    () => importedData.agenda.filter((item) => !item.tema || !item.orador || !item.congregacao || !item.presidente || !item.leitor).length,
    [],
  );
  const emptyAgendaCount = useMemo(
    () => importedData.agenda.filter((item) => !item.tema && !item.orador && !item.congregacao).length,
    [],
  );
    ...
    <div className="min-h-screen bg-[#f7f4ec] text-[#173b40] selection:bg-[#d6aa62] selection:text-[#173b40]">
      ...
      <main className="mx-auto max-w-7xl px-5 py-10 md:px-8 md:py-14">
        <section className="mb-10 grid items-end gap-8 lg:grid-cols-[1fr_auto]">
          ...
          <div className="rounded-2xl border border-[#d9d0bd] bg-white/70 px-5 py-4 text-sm text-[#5c6b69] shadow-sm"><span className="font-semibold text-[#173b40]">Atualização pública</span><br />Consulte os detalhes antes da reunião.</div>
        </section>

        <section className="mb-10 grid gap-4 sm:grid-cols-3" aria-label="Resumo da agenda">
          {[{ icon: CalendarDays, value: importedData.agenda.length, label: "programações registradas" }, { icon: Users, value: importedData.leitores.length, label: "leitores cadastrados" }, { icon: Clock3, value: importedData.themes.length, label: "temas disponíveis" }].map(({ icon: Icon, value, label }) => (
            <div key={label} className="rounded-2xl border border-[#d9d0bd] bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md"><Icon className="mb-4 h-5 w-5 text-[#a47b35]" /><p className="text-3xl font-semibold text-[#173b40]">{value}</p><p className="mt-1 text-sm text-[#6d7773]">{label}</p></div>
          ))}
        </section>
        ...
        <section className="mb-10 rounded-2xl border border-[#d9d0bd] bg-white p-4 shadow-sm md:p-5" aria-label="Filtros da agenda">
          <div className="grid gap-3 md:grid-cols-[1fr_240px_auto]">
            <label className="relative block"><span className="sr-only">Buscar na agenda</span><Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-[#8b938e]" /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar por tema, orador ou congregação" className="h-11 w-full rounded-lg border border-[#d9d0bd] bg-[#fcfbf7] pl-10 pr-3 text-sm outline-none transition focus:border-[#a47b35] focus:ring-2 focus:ring-[#a47b35]/20" /></label>
            <label className="relative block"><span className="sr-only">Filtrar por mês</span><select value={selectedMonth} onChange={(event) => setSelectedMonth(event.target.value)} className="h-11 w-full appearance-none rounded-lg border border-[#d9d0bd] bg-[#fcfbf7] px-3 pr-9 text-sm outline-none transition focus:border-[#a47b35] focus:ring-2 focus:ring-[#a47b35]/20"><option>Todos os meses</option>{months.map((month) => <option key={month}>{month}</option>)}</select><ChevronDown className="pointer-events-none absolute right-3 top-1/2 h-4 w-4 -translate-y-1/2 text-[#8b938e]" /></label>
        ...
        <section className="space-y-10">
          {Object.entries(groupedAgenda).map(([month, items]) => <div key={month}><div className="mb-4 flex items-center gap-3"><h2 className="font-display text-3xl text-[#173b40]">{month}</h2><div className="h-px flex-1 bg-[#d9d0bd]" /></div><div className="grid gap-4 lg:grid-cols-2">{items.map((item, index) => <article key={`${item.data}-${item.tema}-${index}`} className="rounded-2xl border border-[#d9d0bd] bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-sm font-semibold uppercase tracking-[0.12em] text-[#a47b35]">{item.data}</p><h3 className="mt-2 font-display text-2xl leading-snug text-[#173b40]">{item.tema || "Programação a definir"}</h3></div>{item.temaNum && <span className="rounded-full bg-[#f0e7d4] px-3 py-1 text-xs font-semibold text-[#8c672d]">Tema {item.temaNum}</span>}</div><div className="mt-5 grid gap-3 border-t border-[#eee9dd] pt-4 text-sm text-[#5c6b69] sm:grid-cols-2"><p><strong className="font-medium text-[#173b40]">Orador:</strong> {item.orador || "A definir"}</p><p><strong className="font-medium text-[#173b40]">Presidente:</strong> {item.presidente || "A definir"}</p><p><strong className="font-medium text-[#173b40]">Leitor:</strong> {item.leitor || "A definir"}</p><p className="flex items-start gap-1"><MapPin className="mt-0.5 h-4 w-4 shrink-0 text-[#a47b35]" /><span>{item.congregacao || "Local a definir"}</span></p></div>{(item.telefone || item.obs) && <div className="mt-4 rounded-lg bg-[#f7f4ec] px-3 py-2 text-xs text-[#6d7773]">{item.telefone && <span>{item.telefone}</span>}{item.telefone && item.obs && <span> · </span>}{item.obs && <span>{item.obs}</span>}</div>}</article>)}</div></div>)}
          {filteredAgenda.length === 0 && <div className="rounded-2xl border border-dashed border-[#cfc3aa] bg-white p-10 text-center text-[#6d7773]"><Search className="mx-auto h-8 w-8 text-[#a47b35]" /><p className="mt-3 font-medium text-[#405653]">Nenhuma programação encontrada</p><p className="mt-1 text-sm">Tente ajustar os filtros ou limpar a busca.</p>{hasFilters && <button type="button" onClick={clearFilters} className="mt-4 rounded-lg bg-[#173b40] px-4 py-2 text-sm font-medium text-white transition hover:bg-[#28565a] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#d6aa62]">Limpar filtros</button>}</div>}
        </section>
